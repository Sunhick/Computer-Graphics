...
void init_sdl() {
  ...
  if (!music)
    Fatal("cannot load ./sounds/truck.wav");

  crash = Mix_LoadWAV("sounds/smash.wav");
  if (!music)
    Fatal("cannot load ./sounds/smash.wav");

  screech = Mix_LoadWAV("sounds/squeal.wav");
  if (!music)
    Fatal("cannot load ./sounds/squeal.wav");
...
void SetupView() {
  if (mode == WORLD_VIEW) {
    camera = (struct Point3d){.x = -2 * dim * Sin(th) * Cos(ph),
                              .y = +2 * dim * Sin(ph),
  ...
  // First person view
    ...
    gluLookAt(fcamera.x, fcamera.y, fcamera.z, flookAt.x + fcamera.x,
              flookAt.y + fcamera.y, flookAt.z + fcamera.z, fupVector.x,
              fupVector.y, fupVector.z);
  } else if (mode == DRIVE) {
    ...
    gluLookAt(tpos[0] - behindX + heightX, tpos[1] - behindY + heightY,
              tpos[2] - behindZ + heightZ, tpos[0], tpos[1], tpos[2], upVec.x,
              upVec.y, upVec.z);
  }
}
